        ...
        if func_name == 'executar_analise_yara_completa':
            ...
            if YARA_METRICS['total_analyses'] > 0:
                YARA_METRICS['average_analysis_time'] = (
                    (YARA_METRICS['average_analysis_time'] * (YARA_METRICS['total_analyses'] - 1) + execution_time) /
...
YARA_DEPENDENCIES_OK = YARA_DEPENDENCIES_OK and YARA_MODULE_OK
...
def carregar_regras_yara_com_cache():
    ...
    try:
        ...
        if (YARA_CACHE_TIMESTAMP is None or 
            timestamp_mais_recente > YARA_CACHE_TIMESTAMP or 
            YARA_RULES is None):
            ...
            if rules_dict:
                YARA_RULES = yara_module.compile(filepaths=rules_dict)
                YARA_ENABLED = True
                YARA_CACHE_TIMESTAMP = timestamp_mais_recente
                print(f"[YARA] ✅ {regras_carregadas} regras carregadas ({regras_erro} erros)")
                if regras_erro > 0:
                    print(f"[YARA] ⚠️ {regras_erro} regras com problemas foram ignoradas")
            else:
                print("[YARA] ❌ Nenhuma regra YARA válida encontrada")
                YARA_ENABLED = False
        else:
            print(f"[YARA] ♻️ Usando cache de regras (timestamp: {timestamp_mais_recente})")
...
def filtrar_deteccoes_inteligente(deteccoes):
    ...
    if falsos_positivos > 0:
        print(f"[YARA-FILTER] 🧹 {falsos_positivos} possíveis falsos positivos filtrados")
...
def classificar_severidade_deteccao(deteccao):
    ...
    if any(palavra in regra for palavra in ['ransomware', 'trojan', 'backdoor', 'rootkit']):
        return 'CRÍTICA'
...
def gerar_relatorio_yara_melhorado(deteccoes_yara):
    ...
    deteccoes_classificadas.sort(key=lambda x: ordem_severidade.get(x['severidade'], 0), reverse=True)
    ...
    relatorio_texto = f"🚨 RELATÓRIO YARA AVANÇADO - {len(deteccoes_yara)} DETECÇÕES:\n\n"
    ...
    for severidade in ['CRÍTICA', 'ALTA', 'MÉDIA', 'BAIXA']:
        deteccoes_sev = [d for d in deteccoes_classificadas if d['severidade'] == severidade]
        ...
        for regra, deteccoes_regra in regras_sev.items():
            relatorio_texto += f"   {contador}. {regra} ({len(deteccoes_regra)} arquivo(s))\n"
