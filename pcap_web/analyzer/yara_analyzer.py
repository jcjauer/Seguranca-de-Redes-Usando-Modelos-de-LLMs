...
try:
    ...
    # Verificar se tem o método compile
    if hasattr(yara_module, "compile"):
        YARA_MODULE_OK = True
        print(
            f"[YARA] ✅ YARA {getattr(yara_module, 'YARA_VERSION', 'versão desconhecida')} carregado com sucesso"
        )
    else:
        print("[YARA] ❌ Módulo yara não tem método 'compile'")
        YARA_MODULE_OK = False
except ImportError as e:
    print(f"[YARA] ❌ Erro ao importar yara-python: {e}")
    print("[YARA] 💡 Instale com: pip install yara-python")
    YARA_MODULE_OK = False
except Exception as e:
    print(f"[YARA] ❌ Erro inesperado ao importar yara: {e}")
    YARA_MODULE_OK = False
...
def carregar_regras_yara_com_cache():
    ...
    try:
        ...
        if (
            ...
            if rules_dict and YARA_MODULE_OK:
                try:
                    YARA_RULES = yara_module.compile(filepaths=rules_dict)
                    YARA_ENABLED = True
                    YARA_CACHE_TIMESTAMP = timestamp_mais_recente
                    YARA_METRICS["rules_loaded_count"] = regras_carregadas
                    print(
                        f"[YARA] ✅ {regras_carregadas} regras carregadas com sucesso"
                    )
                    if regras_erro > 0:
                        print(
                    ...
                    print(f"[YARA] ❌ Erro ao compilar regras YARA: {e}")
                    YARA_ENABLED = False
                    YARA_RULES = None
            else:
                if not YARA_MODULE_OK:
